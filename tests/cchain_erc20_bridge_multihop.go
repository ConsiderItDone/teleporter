...
func CChainERC20BridgeMultihopGinkgo() {
	CChainERC20BridgeMultihop(&network.LocalNetwork{})
}

func CChainERC20BridgeMultihop(network network.Network) {
	subnets := network.GetSubnetsInfo()
	Expect(len(subnets)).Should(BeNumerically(">=", 3))
	subnetAInfo := subnets[0]
	cchainInfo := subnets[1]
	...
	cchainInfo = utils.SubnetTestInfo{
		...
		TeleporterMessenger:       cchainTeleporterMessengerContract,
	}
	...
	cchainTeleporterMessenger, err := teleportermessenger.NewTeleporterMessenger(
		cchainTeleporterMessengerAddr,
		cchainInfo.ChainRPCClient,
	)
	Expect(err).Should(BeNil())
	...
	erc20BridgeAddressB, erc20BridgeB := localUtils.DeployERC20Bridge(ctx, fundedKey, cchainInfo)
	...
	receipt, messageID := cchainSubmitCreateBridgeToken(
		ctx,
		subnetAInfo,
	...
	bridgeTokenB, err := bridgetoken.NewBridgeToken(bridgeTokenSubnetBAddress, cchainInfo.ChainRPCClient)
	Expect(err).Should(BeNil())
	...
	receipt, messageID = cchainBridgeToken(
		ctx,
		subnetAInfo,
		cchainInfo.BlockchainID,
		erc20BridgeAddressB,
...
func cchainSubmitCreateBridgeToken(
	ctx context.Context,
