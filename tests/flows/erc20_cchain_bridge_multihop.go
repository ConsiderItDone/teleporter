...
func ERC20CChainBridgeMultihop(network interfaces.Network) {
	subnetAInfo, cchainInfo, subnetCInfo := utils.GetThreeSubnets(network)
	...
	erc20BridgeAddressB, erc20BridgeB := utils.DeployERC20Bridge(ctx, fundedKey, cchainInfo)
	...
	bridgeTokenB, err := bridgetoken.NewBridgeToken(bridgeTokenSubnetBAddress, cchainInfo.RPCClient)
	Expect(err).Should(BeNil())
