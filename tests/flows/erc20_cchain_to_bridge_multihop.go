...
func ERC20CChainToBridgeMultihop(network interfaces.Network) {
	subnetAInfo, subnetBInfo, cchainInfo := utils.GetThreeSubnets(network)
	...
	erc20BridgeAddressC, erc20BridgeC := utils.DeployERC20Bridge(ctx, fundedKey, cchainInfo)
	...
	bridgeTokenC, err := bridgetoken.NewBridgeToken(bridgeTokenSubnetCAddress, cchainInfo.RPCClient)
	Expect(err).Should(BeNil())
	...
	Expect(err).Should(BeNil())
	Expect(event.DestinationBlockchainID[:]).Should(Equal(cchainInfo.BlockchainID[:]))
	...
	deliveryReceipt = network.RelayMessage(ctx, receipt, subnetAInfo, cchainInfo, true)
	receiveEvent, err = utils.GetEventFromLogs(
		deliveryReceipt.Logs,
		cchainInfo.TeleporterMessenger.ParseReceiveCrossChainMessage)
	Expect(err).Should(BeNil())
	...
	// Check Teleporter message received on the destination
	delivered, err = subnetATeleporterMessenger.MessageReceived(&bind.CallOpts{}, cchainInfo.BlockchainID, messageID)
	Expect(err).Should(BeNil())
	Expect(delivered).Should(BeTrue())
